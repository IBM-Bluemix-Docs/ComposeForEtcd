...
## 升级版本
 ...
 如果服务已经处于最新可用版本，那么面板会显示当前版本信息。如果有新版本的数据库可用，那么会出现一个下拉菜单，允许您选择要升级到的版本。
...
您可能会发现需要更改服务的密码。您可以在_更改密码_面板中执行此操作。 

可以使用为您创建的随机生成的密码，或者可以在字段中输入自己的密码。要重新生成新的随机密码，请单击字段右侧的方块。 
...
### 更新已连接的应用程序
更改密码将使现有连接字符串失效并生成新的连接字符串。这会导致服务中断，直到连接的应用程序使用新的连接字符串进行更新为止。
...
**注：**尽管 IP 条目支持 IPv6，但是当前没有任何 Compose 部署可用于 IPv6 网络，因此无法对这些地址进行过滤。

### 网络掩码
要允许来自指定 IP 地址范围的连接，请使用网络掩码。使用网络掩码时，必须完全指定 IP 地址。例如，这意味着输入 `192.168.1.0/24` 而不是 `192.168.1/24`。

### 描述
*描述*可以是用于标识白名单条目的任何文本 - 例如，客户名称、项目标识或员工编号。描述字段是必需的。

### Compose 服务
白名单条目会自动添加到 Compose 的服务器，以允许它们进行连接。

### 除去列入白名单的 IP 地址
要从白名单中除去 IP 地址或网络掩码，请单击相应行中的*除去 IP*。除去白名单上的所有条目后，将禁用白名单，并且 TCP 访问门户网站将接受所有 IP 地址。
